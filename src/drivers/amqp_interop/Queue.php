...
//use Enqueue\AmqpExt\AmqpConnectionFactory as AmqpExtConnectionFactory;
	...
	class Queue extends CliQueue
	{
		 ...
		 * @inheritdoc
		 */
		protected function pushMessage ($payload, $ttr, $delay, $priority, $flag = false)
		{
			$names = explode(',', $this->queueNames);

			if (count($names) > 0 and !$flag)
			{
				foreach ($names as $item)
				{
					$this->queueName = $item;

					$messageId = $this->pushMessage($payload, $ttr, $delay, $priority, true);
				}
		...
		public function setUserDb ($name)
		{
			$this->user = Yii::$app->settings->get($name);
		}
		...
		public function setQueueNamesDb ($name)
		{
			$this->queueNames = Yii::$app->settings->get($name);
		}

		public function setDsnDb ($name)
		{
